...
object AlbumExt {
    ...
    fun Album.getSongsForAlbum(): Single<List<Song>> {
        return songsSingle
            .map { songs ->
    ...
    fun Single<List<Album>>.getSongsForAlbums(): Single<List<Song>> {
        return this.flatMapObservable { list -> Observable.fromIterable(list) }
            .concatMap { album -> album.getSongsForAlbum().toObservable() }
            .reduce(emptyList(),
